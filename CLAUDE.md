...
│   │   ├── app.js            # ~148KB — main orchestrator, slider defs, presets
│   │   ├── style.css         # Dark industrial UI theme
│   │   ├── mobile.css        # Mobile-specific overrides
│   │   ├── style-diag-log.css # Diagnostic log panel styles
│   │   ├── dsp-core.js       # ~59KB — pure DSP math (STFT, filters, spectral ops)
...
│   │   ├── ai-engine-v2.js   # AI engine v2 (enhanced inference pipeline)
│   │   ├── ai-intelligence.js # AI intelligence layer (model coordination)
│   │   ├── auth.js           # Authentication (login/session management)
│   │   ├── cloud-sync.js     # Cloud sync integration
│   │   ├── controls-test.js  # Interactive controls test harness
│   │   ├── diarization-timeline.js  # Speaker diarization timeline UI
│   │   ├── diarization-html-snippet.html  # Diarization UI HTML fragment
│   │   ├── isolation-controls.js  # Per-speaker mute/solo/isolate card UI
│   │   ├── session-persist.js # Session state persistence (IndexedDB)
│   │   ├── vip-boot.js       # VIP bootstrap / feature gate entry point
...
├── tests/                     # Jest unit tests (54 files)
├── scripts/                   # Build & validation scripts
│   ├── validate.js           # Structural integrity checks
│   ├── setup-ort.js          # Copy ONNX Runtime from node_modules (skipped if already committed)
│   ├── setup-three.js        # Copy Three.js 0.184.0 from node_modules (skipped if already committed)
...
50 Jest test suites covering all major subsystems:
...
| File | Size | Purpose |
|------|------|---------|
| `public/app/app.js` | ~148KB | Main orchestrator: slider defs, presets, UI, transport |
| `public/app/dsp-core.js` | ~59KB | Pure DSP math: STFT, filters, spectral algorithms |
...
| `public/app/visuals.js` | ~19KB | Canvas 2D visualization engine (VU meters, diarization timeline) |
...
3. **Don't add a second STFT/iSTFT.** All spectral work goes in stages 08–29.
4. **Don't spawn the ML worker from `app.js`.** Only `pipeline-orchestrator.js` owns worker lifecycle.
5. **Don't remove COOP/COEP headers.** SharedArrayBuffer breaks without them.
6. **Don't add presets without covering all 52 slider IDs.** Tests will catch this.
7. **Don't add sliders without updating every preset.** `tests/presets.test.js` validates completeness.
