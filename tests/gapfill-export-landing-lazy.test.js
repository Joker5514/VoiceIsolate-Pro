/**
 * Gap-fill: export cancel overlay, landing cancel, complete Engineer sliders.
 */
'use strict';
...
describe('Gap-fill export + landing cancel + complete sliders', () => {
  ...
  test('Engineer mounts the complete slider rack without lazy closed-panel deferral', () => {
    expect(appJs).toMatch(/Mount the complete 67-control rack/);
    expect(appJs).toMatch(/for \(const s of RENDER_SLIDERS\)/);
    expect(appJs).toMatch(/this\._sliderFilterMode = 'all'/);
    expect(appJs).not.toMatch(/_pendingSlidersByPanel/);
    expect(appJs).not.toMatch(/_flushPendingSlidersFor|_wireLazySliderPanel/);
    expect(appJs).not.toMatch(/details && !details\.open/);
  });
