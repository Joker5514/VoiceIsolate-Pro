...
function extractSlidersFromSrc() {
  // Extract only from the SLIDERS constant block (between const SLIDERS = { and const SLIDER_MAP)
  const blockMatch = appSrc.match(/const SLIDERS = \{([\s\S]*?)\};\n*const SLIDER_MAP/);
  const block = blockMatch ? blockMatch[1] : appSrc;
  const idMatches = [...block.matchAll(/id\s*:\s*'([^']+)'/g)];
...
describe('STAGES array', () => {
  ...
  test('contains exactly 32 stage entries (Octa-Pass pipeline)', () => {
...
describe('VoiceIsolatePro class structure', () => {
  ...
  test('pipeline has 32 stages to match STAGES array', () => {
    // The comment in source reads "32-Stage Octa-Pass Pipeline"
    expect(appSrc).toContain('32-Stage Octa-Pass Pipeline');
...
describe('estVoices() logic', () => {
  ...
  test('returns "1" for 3–9 active half-second windows', () => {
    // 4 active half-second windows at 44100 = 4 * 22050 = 88200 samples (= 2 seconds)
    const d = new Float32Array(44100 * 2).fill(0.5);
